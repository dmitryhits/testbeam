...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   TH1F* h_Integral_short_50_calib = new TH1F("h_Integral_short_50_calib","Integral short, 50 bins - calibration events",2024,-600,+600);
   TH1F* h_Integral_short_50_nocalib = new TH1F("h_Integral_short_50_nocalib","Integral short, 50 bins - NON-calibration events",2024,-600,+600);
...
//	if(ientry<54000){ // limits for run 13
	...
	if(isCalib > 0) h_Integral_short_50_calib->Fill(integral_short_50); //50
	else h_Integral_short_50_nocalib->Fill(integral_short_50); //50
   ...
   c1->cd(9);
   h_Integral_short_50_calib->Draw();
   c1->cd(10);
   h_Integral_short_50_nocalib->Draw();

// write the histograms in the root file
   h_1st_trigger_time->Write();
   h_2nd_trigger_time->Write();
   h_Integral_short_50->Write();
   h_Integral_short_50_calib->Write();
   h_Integral_short_50_nocalib->Write();
...
int main(int argc, char* argv[]){
  ...
  cout<<"calling rec on file " << infile << ".root" << std::endl;
  rec t(infile,tim);

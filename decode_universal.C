...
void decode(char *filename) {
  ...
  for (n=0 ; fread(&header, sizeof(header), 1, f) > 0 && n < 2; n++) {
    //  cout << "30" << endl;
    ...
    month = header.month;
    day = header.day;
    if (n%1000 == 0)cout << "year "<< year << ": month " << month << ": day " << day << endl; 
    time_stamp = header.hour*3600+header.minute*60+header.second+header.millisecond/1000.; 
    if (n%1000 == 0) cout << "millisecond " << header.millisecond << endl;
    for (Int_t i=0; i<1024; i++)
      t[i] = (Double_t) header.time[i];

    //  cout << "3" << endl;
    cout << "waveform size : " <<    sizeof(waveform)/4*n_channels << endl;

    fread(&waveform, sizeof(waveform)/4, 1, f);
    fread(&waveform, sizeof(waveform)/4, 1, f);
    fread(&waveform, sizeof(waveform)/4, 1, f);
    fread(&waveform, sizeof(waveform)/4, 1, f);
